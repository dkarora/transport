...
class FileUploadComponent extends Object{
  ...
  function _processFile(){
    ...
    if(move_uploaded_file($this->uploadedFile['tmp_name'], $target_path)){
	  ...
	  // step through fields to make it *useful*
	  foreach ($this->fields as $arrKey => $colName)
	  {
		// we already did that.
		if ($arrKey == 'name')
			continue;
		
		$save_data[$colName] = $this->uploadedFile[$arrKey];
	  }
      /*$save_data[$this->fields['type']] = $this->uploadedFile['type'];
      $save_data[$this->fields['size']] = $this->uploadedFile['size'];
	  if (!empty($this->fields['friendly_name']))
		$save_data[$this->fields['friendly_name']] = $this->uploadedFile['friendly_name'];*/
