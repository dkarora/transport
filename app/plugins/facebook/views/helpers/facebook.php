...
class FacebookHelper extends AppHelper {
  /**
    ...
    */
	// add Javascript for cake 1.2 compatibility
  ...
  // these tags should already be escaped, don't escape them again
  var $__ogTagsNoEscape = array('og:title');
  ...
  /**
    ...
    */
  function html(){
    return '<html xmlns="http://www.w3.org/1999/xhtml" xmlns:fb="http://www.facebook.com/2008/fbml" xmlns:og="http://ogp.me/ns#">';
  }
  
  private function __ogTag($name = null, $content = null)
  {
	...
	if (in_array($name, $this->__ogTagsNoEscape))
		$options = array('escape' => false);
	
	return $this->Html->meta(array('property' => $name, 'content' => $content), null, $options);
  }
  
  function ogTags($tags = array())
  {
	if (empty($tags))
		return '';
	...
	foreach ($tags as $name => $content)
		$meta .= $this->__ogTag($name, $content) . "\n";
  ...
  function share($url = null, $options = array()){
    ...
    if($options['fbxml']){
      ...
      $retval .= $this->Javascript->link($this->__fbShareScript);
    }
  ...
  function init($options = array()){
    if(FacebookInfo::getConfig('appId')){
		...
		// this plugin was designed for cake 1.3 but we use 1.2.
