			...
			if ($session->check('User.id'))
			{
				...
				if ($isAttendee && !$locked)
				{
					echo $html->link(
						...
						//echo $html->link($html->image('attendee-management/email-invoice.png', array('title' => 'Email Invoice', 'alt' => 'Envelope')), array('controller' => 'attendees', 'action' => 'email_invoice', $id), array('escape' => false), 'Are you sure you want to send an invoice to attendee'. $attendee['Attendee']['id'] . '?');
				
						echo $html->link(
						'Pay by Credit Card &raquo;',
						'https://quikpayasp.com/umass/commerce_manager/payer.do?orderType=BAYSTATE',
						array('class' => 'button-link reject full-width slim', 'id' => 'enroll-button'), null, array('escape' => false)
						);
